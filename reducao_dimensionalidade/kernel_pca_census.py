...
previsores desta base terá melhores resultados do que utilizando o Kernel pca para
...
REDUÇÃO DE DIMENSIONALIDADE
"""
# Criação do objeto da classe Kernel PCA 
kpca = KernelPCA(n_components=6, kernel='rbf')  # n_componentes é o valor de número de atributos que queremos
previsores_treinamento = kpca.fit_transform(previsores_treinamento)
previsores_teste = kpca.transform(previsores_teste)  # como utilizamos o fit_transform, o método já se adaptou aos dados, por isso usamos transform apenas, para usar a mesma escala
...
componentes = kpca.explained_variance_  # este método retorna os coeficientes de cada atributo selecionado (escolhido no método durante a criação do objeto da classe PCA)
